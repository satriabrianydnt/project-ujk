        ...
        <div class="bg-white border border-gray-200/70 rounded-2xl shadow-sm overflow-hidden flex flex-col">
            ...
            @if (isset($kategoris) && count($kategoris) > 0)
                        ...
                        <thead class="bg-white border-b border-gray-200/70 sticky top-0 z-10">
                            <tr>
                                <th scope="col"
                                    ...
                                    No</th>
                                <th scope="col"
                                    class="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest">Nama
                                    Kategori</th>
                                <th scope="col"
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($kategoris as $index => $kategori)
                                <tr class="hover:bg-indigo-50/30 transition-colors group">
                                    ...
                                    <td class="px-6 py-4">
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-[12px] font-bold bg-slate-100 text-slate-700 border border-slate-200/60 uppercase tracking-tight">
                                            <svg class="w-3.5 h-3.5 mr-1.5 text-slate-400" fill="none"
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span
                                            class="inline-flex items-center justify-center px-2.5 py-1 rounded-lg text-xs font-bold {{ $kategori->barang_count > 0 ? 'bg-indigo-50 text-indigo-700 border border-indigo-100' : 'bg-gray-50 text-gray-500 border border-gray-200' }}">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div
                                            ...
                                            <button type="button"
                                                onclick="toggleModal('editKategoriModal-{{ $kategori->id }}')"
                                            ...
                                            <form action="{{ route('kategori-barang.destroy', $kategori->id) }}"
                                                method="POST"
                                                onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ({{ $kategori->nama_kategori }}) ini?');">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                ...
                <div class="block md:hidden divide-y divide-gray-100/80 bg-gray-50/30">
                    @foreach ($kategoris as $index => $kategori)
                        <div class="p-4 hover:bg-indigo-50/50 transition-colors flex justify-between items-center">

                            <div class="flex flex-col gap-1.5">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-bold bg-slate-100 text-slate-700 border border-slate-200/60 uppercase tracking-tight w-fit">
                                ...
                                <span class="text-[10px] font-medium text-gray-500 ml-1">
                                    Total: <strong
                                        class="{{ $kategori->barang_count > 0 ? 'text-indigo-600' : 'text-gray-400' }}">{{ $kategori->barang_count }}
                                        Item</strong>
                                </span>
                            </div>

                            <div class="flex gap-1">
                                <button type="button" onclick="toggleModal('editKategoriModal-{{ $kategori->id }}')"
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center w-full gap-4">
                        <div class="text-sm text-gray-500 font-medium text-center md:text-left">
                            @if (request()->filled('search'))
                                Hasil pencarian: <span class="font-bold text-indigo-600">{{ $kategoris->total() }}</span>
                                Data ditemukan
                                <span class="text-gray-400 mx-1">|</span>
                                Halaman <span class="font-bold text-gray-900">{{ $kategoris->currentPage() }}</span>
                            @else
                                Menampilkan <span
                                    class="font-bold text-gray-900">{{ $kategoris->firstItem() ?? 0 }}</span>
                                hingga <span class="font-bold text-gray-900">{{ $kategoris->lastItem() ?? 0 }}</span>
                                dari <span class="font-bold text-indigo-600">{{ $kategoris->total() }}</span> Data
                            @endif
                        </div>
            ...
            @else
                <div class="flex flex-col items-center justify-center px-6 py-16 sm:py-24 text-center bg-white">
                    ...
                    <p class="text-sm text-gray-500 max-w-sm mx-auto mb-6 leading-relaxed">Anda belum menambahkan data
                        kategori apa pun ke dalam sistem.</p>
                    <button onclick="toggleModal('addKategoriModal')"
